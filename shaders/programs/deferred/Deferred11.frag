...
#include "/libs/Shadow.glsl"
    ...
    vec3 percentageCloserSoftShadow(
        vec3 worldPos, vec3 worldDir, vec3 worldGeoNormal, float NdotL, float viewLength, float lightFactor, float smoothness, float porosity, float skyLight, vec2 noise
    ) {
        vec3 result = vec3(0.0);
        if (weatherStrength < 0.999) {
            float leakFixFactor = pow2(1.0 - skyLight);
            worldPos -= worldDir * clamp(viewLength, 0.0, 1.0) * leakFixFactor;
            vec3 sssShadowCoord = worldPosToShadowCoordNoDistort(worldPos);
            float normalOffsetLen = (viewLength * 2e-3 + 2e-2) * (1.0 + sqrt(1.0 - NdotL));
            vec3 normalOffset =
                mat3(shadowModelViewProj0, shadowModelViewProj1, shadowModelViewProj2) *
                (worldGeoNormal * (normalOffsetLen * 4096.0 / realShadowMapResolution + 0.5 * leakFixFactor));
            normalOffset.z *= 0.1;
            ...
            basicShadowCoordNoDistort.z -= shadowProjection[2].z * 0.045 * leakFixFactor / (NdotL + 1e-6);
        ...
        finalColor.rgb *= gbufferData.albedo.rgb;
            ...
            float shadowLightFactor = 1.0;
                ...
                shadow *= percentageCloserSoftShadow(
                    worldPos, worldDir, worldGeoNormal, NdotL, 1.0 / viewLength, shadowLightFactor,
